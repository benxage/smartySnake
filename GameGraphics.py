...
class snakeGame:
    def __init__(self, width=72, height=46, display=False, player=None):
        self.game = Interface.Interface(width, height)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP or event.key == ord('w'):
                    key = Interface.Direction.UP
                if event.key == pygame.K_DOWN or event.key == ord('s'):
                    key = Interface.Direction.DOWN
                if event.key == pygame.K_LEFT or event.key == ord('a'):
                    key = Interface.Direction.LEFT
                if event.key == pygame.K_RIGHT or event.key == ord('d'):
                    key = Interface.Direction.RIGHT
